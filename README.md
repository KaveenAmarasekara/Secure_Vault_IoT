...
**Purpose**: Detect light when the door opens.

#### Input 2 – Authentication Dial
- Connect the **middle pin of the potentiometer** to another **analog pin**.

**Purpose**: Acts as a **secret dial-based authentication code**.
...
**Purpose**: Confirms the entered authentication value.
...
**Why?**  
`delay()` blocks the microcontroller and prevents it from reading inputs. Instead, use `millis()` for non-blocking timing.
...
**Purpose**: Send alerts and logs to the cloud.
...
#### MQTT Monitoring
1. Open the **HiveMQ WebSocket Client** on your laptop.
2. Subscribe to the project topic.
...
#### Testing Procedure
1. Trigger the LDR sensor using your hand or light.
2. Wait 15 seconds without authentication.
